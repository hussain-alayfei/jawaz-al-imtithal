# مِعيار | فحص المخططات قبل الترخيص

مِعيار هو منتج عربي للفحص الاستباقي لملف IFC قبل تقديم طلب
...
عند رفع ملف متوافق مع عقد الاختبار `MIYAR-IFC-1.0`، تنفّذ الواجهة المراحل
...
- يحسب مؤشر الجاهزية بالصيغة الشفافة
  `عدد القواعد المطابقة ÷ إجمالي القواعد × 100`. لا تمنح حالة «معلومات غير
  مكتملة» رصيدًا جزئيًا.
...
يتضمن المجلد ثمانية ملفات صالحة، وثمانية ملفات أخطاء مقصودة، وملفات JSON
...
الملفات الصالحة عينات QA دلالية اصطناعية وليست هندسة إنتاجية. تستخدم
كتالوجًا حتميًا مسمى من الجدران والبلاطات والنوافذ والعناصر الإنشائية
والتجهيزات، مع خصائص فئة ومنطقة وفهرس واستخدام اختبار فقط. لذلك تتحقق
الاختبارات من خط المعالجة والقواعد والأعداد، ولا تدعي أنها نماذج BIM
هندسية فعلية.
